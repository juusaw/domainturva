...
func TestDecide_BaselineFirstFailureSilent(t *testing.T) {
	...
	out, err := e.Process(context.Background(), r, 1)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDecide_RetriesNotYetExhausted(t *testing.T) {
	now := time.Date(2026, 5, 8, 12, 0, 0, 0, time.UTC)
	e, mem := newEngine(now)
	...
	out, _ := e.Process(context.Background(), r, 3)
	if out.NewState.Status != string(checker.StatusUp) {
		t.Fatalf("status should remain up before retries exhausted, got %s", out.NewState.Status)
	}
	if len(out.Alerts) != 0 {
		t.Fatalf("no alerts before flip")
	}
}

func TestDecide_UpToDownEmitsAlert(t *testing.T) {
	now := time.Date(2026, 5, 8, 12, 0, 0, 0, time.UTC)
	e, mem := newEngine(now)
	mem.UpsertSiteState(context.Background(), storage.SiteState{
		SiteName: "a", Status: string(checker.StatusUp), LastStatusChangeAt: now.Add(-time.Hour),
	...
	r := checker.CheckResult{SiteName: "a", Type: checker.CheckHTTP, Status: checker.StatusDown, Error: "x"}
	out, _ := e.Process(context.Background(), r, 1)
	if out.NewState.Status != string(checker.StatusDown) {
		t.Fatalf("should flip to down")
	}
